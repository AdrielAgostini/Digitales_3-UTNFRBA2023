...
static irqreturn_t driver_isr(int irq, void *devid) {
  ...
  ////dev_info(&pdev->dev, "Entro al IRQ Handler\n");
	...
	if (irq_status & I2C2_IRQSTATUS_RRDY)
	{
      ...
      ////dev_info(&pdev->dev, "IRQ handler source -> TX_dato: 0x%x\n",tx_data);
		}
  ...
  //dev_info(&pdev->dev, "IRQ Handler OK\n");
	return IRQ_HANDLED;
}

static int i2c2_MPU6050_open(struct inode *inode, struct file *file) {
  uint16_t aux = 0;
	////dev_info(&pdev->dev, "Inicializacion del MPU6050 iniciada\n");
  ...
  //dev_info(&pdev->dev, "Inicializacion del MPU6050 terminada\n");
}
